...
#
Job = namedtuple(
    "Job", ["name", "run_time", "working_set_size", "affinity", "time_left"]
)
...
class scheduler:
    def __init__(
        ...
        for entry in job_list.split(","):
            ...
            if len(tmp) != 3:
                print(
                    "bad job description [%s]: needs triple of name:runtime:working_set_size"
                    % entry
                )
                exit(1)
            ...
            print(
                "Job name:%s run_time:%d working_set_size:%d"
            ...
            for entry in affinity.split(","):
                ...
                for cpu in tmp[1].split("."):
                    ...
                    if int(cpu) < 0 or int(cpu) >= num_cpus:
                        print(
                            "bad cpu %d specified in affinity %s" % (int(cpu), affinity)
                        )
                        exit(1)
        ...
        if self.per_cpu_queues:
            ...
            while len(jobs_not_assigned) > 0:
                for cpu in range(num_cpus):
                    ...
                    for job_name in jobs_not_assigned:
                        if (
                            len(self.jobs[job_name].affinity) == 0
                            or cpu in self.jobs[job_name].affinity
                        ):
            ...
            for cpu in range(num_cpus):
                print(
                    "Scheduler CPU %d queue: %s" % (cpu, self.per_cpu_sched_queue[cpu])
                )
            print("")
    ...
    def run(self):
        ...
        if self.solve:
            ...
            for cpu in range(self.num_cpus):
                print(
                    "  CPU %d  utilization %3.2f [ warm %3.2f ]"
                    % (
                        cpu,
                        100.0 * float(self.stats_ran[cpu]) / float(self.system_time),
                        100.0
                        * float(self.stats_ran_warm[cpu])
                        / float(self.system_time),
                    )
                )
            print("")
        return
...
parser.add_option(
    ...
    "--max_run",
    default=100,
    help="max run time of random-gen jobs",
    action="store",
    type="int",
    dest="max_run",
)
parser.add_option(
    ...
    "--max_wset",
    default=200,
    help="max working set of random-gen jobs",
    action="store",
    type="int",
    dest="max_wset",
)
parser.add_option(
    ...
    "--job_list",
    default="",
    help="provide a comma-separated list of job_name:run_time:working_set_size (e.g., a:10:100,b:10:50 means 2 jobs with run-times of 10, the first (a) with working set size=100, second (b) with working set size=50)",
    action="store",
    type="string",
    dest="job_list",
)
parser.add_option(
    ...
    "--per_cpu_queues",
    default=False,
    help="per-CPU scheduling queues (not one)",
    action="store_true",
    dest="per_cpu_queues",
)
parser.add_option(
    ...
    "--affinity",
    default="",
    help="a list of jobs and which CPUs they can run on (e.g., a:0.1.2,b:0.1 allows job a to run on CPUs 0,1,2 but b only on CPUs 0 and 1",
    action="store",
    type="string",
    dest="affinity",
)
...
parser.add_option(
    ...
    "--peek_interval",
    default=30,
    help="for per-cpu scheduling, how often to peek at other schedule queue; 0 turns this off",
    action="store",
    type="int",
    dest="peek_interval",
)
parser.add_option(
    ...
    "--warmup_time",
    default=10,
    help="time it takes to warm cache",
    action="store",
    type="int",
    dest="warmup_time",
)
parser.add_option(
    ...
    "--warm_rate",
    default=2,
    help="how much faster to run with warm cache",
    action="store",
    type="int",
    dest="warm_rate",
)
...
parser.add_option(
    ...
    "--rand_order",
    default=False,
    help="has CPUs get jobs in random order",
    action="store_true",
    dest="random_order",
)
parser.add_option(
    ...
    "--trace",
    default=False,
    help="enable basic tracing (show which jobs got scheduled)",
    action="store_true",
    dest="trace",
)
parser.add_option(
    ...
    "--trace_time_left",
    default=False,
    help="trace time left for each job",
    action="store_true",
    dest="trace_time_left",
)
parser.add_option(
    "-C",
    "--trace_cache",
    default=False,
    help="trace cache status (warm/cold) too",
    action="store_true",
    dest="trace_cache",
)
parser.add_option(
...
S = scheduler(
    ...
    per_cpu_queues=options.per_cpu_queues,
    ...
    cache_rate_cold=1,
    cache_rate_warm=cache_rate_warm,
    cache_warmup_time=cache_warmup_time,
    solve=options.solve,
    trace=do_trace,
    trace_time_left=options.trace_time_left,
    trace_cache=options.trace_cache,
